...
% --- Tables, lists, floats, algorithms ------------------------------------
\usepackage{booktabs}
\usepackage{enumitem}
\usepackage{float}
\usepackage{seqsplit}
...
% Syntax-highlighting palette for code.
\usepackage{listings}
\usepackage{xcolor}
...
% inline monospace helper -- long identifiers are made breakable (no visible
% hyphen) via \seqsplit so they wrap cleanly instead of running into the margin.
% Robust so it survives moving arguments (e.g. \caption in algorithm floats).
...
% bound-literal brackets (no stmaryrd dependency).
% \bl{...} wraps its argument in double brackets, e.g. \bl{x \ge 5} -> [[x >= 5]],
% so call sites give the content once instead of pairing \llbracket ... \rrbracket.
\newcommand{\bl}[1]{[\![#1]\!]}
...
\input{sections/01-what-kind-of-solver}
\input{sections/02-foundations}
\input{sections/03-learning-from-failure}
\input{sections/04-lazy-encoding}
\input{sections/05-putting-it-together}
\input{sections/06-going-faster}
